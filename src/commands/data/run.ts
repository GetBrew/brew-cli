 ...
 *
 * A failed COMMAND is a normal HTTP 200 with `exitCode !== 0` and the
 * message in `output` — the command prints it and exits non-zero so
 * shell scripts compose.
 */
export const dataRunCommand = defineCommand({
  ...
  run: async ({ ctx, args }) => {
    const result = await ctx.client().data.run({
