 ...
 * brand's data (POST /v1/data): find/agg with time ranges + policy-gated
 * writes, composable with jq/grep/sort pipes inside the sandbox. Start
 * with `db ls` and `db schema <table>`. A failed COMMAND is a normal
 * HTTP 200 with `exitCode !== 0` and the message in `output` — the
 * command prints it and exits non-zero so shell scripts compose.
 */
export const dataRunCommand = defineCommand({
  ...
  run: async ({ ctx, args }) => {
    const result = await ctx.client().data.run({ command: args.command ?? '' })
    return {
      data: result,
      human: result.output,
